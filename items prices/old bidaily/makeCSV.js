...
function main(inStr){
	...
	for (i in obj){
		...
		for (j=60; j<obj[i].data.length; j++){
			w_timestamp = String(obj[i].data[j][0]);
			w_price_now = String(obj[i].data[j][1]);
			w_price_one = String(obj[i].data[j-1*2][1]);
			w_price_seven = String(obj[i].data[j-7*2][1]);
			w_price_thirty = String(obj[i].data[j-30*2][1]);
			w_thirty_min = String(getMin(obj[i].data.slice(j-30*2, j)));
			w_thirty_max = String(getMax(obj[i].data.slice(j-30*2, j)));
			w_thirty_average = String(getAvg(obj[i].data.slice(j-30*2, j)));
